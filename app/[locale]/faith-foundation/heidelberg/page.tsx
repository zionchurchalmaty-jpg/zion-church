...
import { heidelbergQuestions } from "@/content/faith-foundation/heidelberg";
...
export async function generateMetadata({
  ...
  const t = await getTranslations({ locale, namespace: "heidelberg" });
...
export default async function HeidelbergPage({
  ...
  const t = await getTranslations("heidelberg");
            ...
            <div className="grid md:grid-cols-3 gap-4">
              <div className="bg-cream rounded-lg p-4">
                <div className="flex items-center gap-3 mb-2">
                  <Heart className="w-5 h-5 text-primary-orange" />
                  <h3 className="font-semibold text-navy text-sm">{t("part1Title")}</h3>
                </div>
                <p className="text-xs text-gray-600">{t("part1Description")}</p>
              </div>
              <div className="bg-cream rounded-lg p-4">
                <div className="flex items-center gap-3 mb-2">
                  <ScrollText className="w-5 h-5 text-primary-orange" />
                  <h3 className="font-semibold text-navy text-sm">{t("part2Title")}</h3>
                </div>
                <p className="text-xs text-gray-600">{t("part2Description")}</p>
              </div>
              <div className="bg-cream rounded-lg p-4">
                <div className="flex items-center gap-3 mb-2">
                  <BookOpen className="w-5 h-5 text-primary-orange" />
                  <h3 className="font-semibold text-navy text-sm">{t("part3Title")}</h3>
                </div>
                <p className="text-xs text-gray-600">{t("part3Description")}</p>
              </div>
            </div>
          </div>
        </div>
      ...
      <section className="py-8 pb-16">
        <div className="max-w-4xl mx-auto px-4 sm:px-6 lg:px-8">
          <div className="space-y-4">
            {heidelbergQuestions.map((item, index) => (
              <div
                key={index}
