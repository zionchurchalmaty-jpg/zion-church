...
import { westminsterShorterQuestions } from "@/content/faith-foundation/westminster-shorter";
...
export async function generateMetadata({
  ...
  const t = await getTranslations({ locale, namespace: "westminsterShorter" });
...
export default async function WestminsterShorterPage({
  params,
}: {
  params: Promise<{ locale: string }>;
}) {
  ...
  const t = await getTranslations("westminsterShorter");
      ...
      <section className="py-8 pb-16">
        <div className="max-w-4xl mx-auto px-4 sm:px-6 lg:px-8">
          <div className="space-y-4">
            {westminsterShorterQuestions.map((item, index) => (
              <div
                key={index}
